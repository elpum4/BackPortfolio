...
from app.models.behaviour.skill import SkillController
...
class AllSkill(Resource):
    def get(self):
        try:
            response = SkillController.get_all()
            # logger.info("collection list successfully retrieved")
        except Exception as e:
            # helpers.log_traceback(e)
            message = "Error retrieving the skill list"
    ...
    def post(self, current_user):
        try:
            response = SkillController.new()
            # logger.info("successfully created")
        except Exception as e:
            # helpers.log_traceback(e)
            message = "Error to created the skill"
            logger.error(f"{message}. Error: {e}")
            return make_response(
                jsonify({"message": message}),
                400,
...
class Skill(Resource):
    def get(self, id):
        try:
            response = SkillController.from_id(id)
            logger.info("collection list successfully retrieved")
        except Exception as e:
            # helpers.log_traceback(e)
    ...
    def delete(self, current_user, id):
        try:
            response = SkillController.delete(id)
            logger.info("successfully Created")
        except Exception as e:
            # helpers.log_traceback(e)
